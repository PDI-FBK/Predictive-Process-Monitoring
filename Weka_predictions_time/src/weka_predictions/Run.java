  ...
  public static void main(String[] args) {
    try {
      ...
      if (args[0].equals("-list-packages")) {
        weka_predictions.core.WekaPackageManager.loadPackages(true, false);
        System.exit(0);
      ...
      if (!noLoad) {
        weka_predictions.core.WekaPackageManager.loadPackages(false, false);
      }
      ...
      if (!noScan) {     
        weka_predictions.core.ClassDiscovery.clearClassCache();
        ArrayList<String> matches = weka_predictions.core.ClassDiscovery.find(schemeToRun);
        ...
        for (int i = 0; i < matches.size(); i++) {
          try {
            Object scheme = java.beans.Beans.instantiate((new Run()).getClass().getClassLoader(),
                matches.get(i));          
            if (scheme instanceof weka_predictions.classifiers.Classifier ||
                scheme instanceof weka_predictions.clusterers.Clusterer ||
                scheme instanceof weka_predictions.associations.Associator ||
                scheme instanceof weka_predictions.attributeSelection.ASEvaluation ||
                scheme instanceof weka_predictions.filters.Filter ||
                scheme instanceof weka_predictions.core.converters.AbstractFileLoader ||
                scheme instanceof weka_predictions.core.converters.AbstractFileSaver ||
                scheme instanceof weka_predictions.core.CommandlineRunnable) {
      ...
      if (scheme instanceof weka_predictions.clusterers.Clusterer) {
        types.add(SchemeType.CLUSTERER);
      }
      if (scheme instanceof weka_predictions.associations.Associator) {
        types.add(SchemeType.ASSOCIATOR);
      }
      if (scheme instanceof weka_predictions.attributeSelection.ASEvaluation) {
        types.add(SchemeType.ATTRIBUTE_SELECTION);
      }
      if (scheme instanceof weka_predictions.filters.Filter) {
        types.add(SchemeType.FILTER);
      }
      if (scheme instanceof weka_predictions.core.converters.AbstractFileLoader) {
        types.add(SchemeType.LOADER);
      }
      if (scheme instanceof weka_predictions.core.converters.AbstractFileSaver) {
        types.add(SchemeType.SAVER);
      }
      if (scheme instanceof weka_predictions.core.CommandlineRunnable) {
      ...
      if (selectedType == SchemeType.CLASSIFIER) {
        weka_predictions.classifiers.AbstractClassifier.runClassifier((weka_predictions.classifiers.Classifier)scheme, options);
      } else if (selectedType == SchemeType.CLUSTERER) {
        weka_predictions.clusterers.AbstractClusterer.runClusterer((weka_predictions.clusterers.Clusterer)scheme, options);
      } else if (selectedType == SchemeType.ATTRIBUTE_SELECTION) {
        weka_predictions.attributeSelection.ASEvaluation.runEvaluator((weka_predictions.attributeSelection.ASEvaluation)scheme, options);
      } else if (selectedType == SchemeType.ASSOCIATOR) {
        weka_predictions.associations.AbstractAssociator.runAssociator((weka_predictions.associations.Associator)scheme, options);
      } else if (selectedType == SchemeType.FILTER) {
        weka_predictions.filters.Filter.runFilter((weka_predictions.filters.Filter)scheme, options);
      } else if (selectedType == SchemeType.LOADER) {
        weka_predictions.core.converters.AbstractFileLoader.
          runFileLoader((weka_predictions.core.converters.AbstractFileLoader)scheme, options);
      } else if (selectedType == SchemeType.SAVER) {
        weka_predictions.core.converters.AbstractFileSaver.
          runFileSaver((weka_predictions.core.converters.AbstractFileSaver)scheme, options);
      } else if (selectedType == SchemeType.COMMANDLINE) {
        ((weka_predictions.core.CommandlineRunnable)scheme).run(scheme, options);
      }
