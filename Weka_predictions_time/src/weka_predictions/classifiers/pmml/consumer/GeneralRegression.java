...
import weka_predictions.core.pmml.PMMLUtils;
...
public class GeneralRegression extends PMMLClassifier
  ...
  enum CumulativeLinkFunction {
    ...
    PROBIT ("probit") {
      double eval(double value, double offset) {
        return weka_predictions.core.matrix.Maths.pnorm(value + offset); 
      }
    },
    CLOGLOG ("cloglog") {
      double eval(double value, double offset) {
  ...
  enum LinkFunction {
    ...
    PROBIT ("probit") {
      double eval(double value, double offset, double trials,
                  double distParam, double linkParam) {
        return weka_predictions.core.matrix.Maths.pnorm(value + offset) * trials;
      }
    };
