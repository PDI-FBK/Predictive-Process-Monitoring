...
import weka_predictions.classifiers.rules.ZeroR;
...
import weka_predictions.core.xml.KOML;
import weka_predictions.core.xml.XStream;
import weka_predictions.experiment.Task;
import weka_predictions.experiment.TaskStatusInfo;
...
public class Classifier extends JPanel implements BeanCommon, Visible,
  ...
  private weka_predictions.classifiers.Classifier m_Classifier = new ZeroR();
  /** Template used for creating copies when building in parallel */
  private weka_predictions.classifiers.Classifier m_ClassifierTemplate = m_Classifier;
   ...
   */
  public void setClassifierTemplate(weka_predictions.classifiers.Classifier c) {
    ...
    if (!(m_ClassifierTemplate instanceof weka_predictions.classifiers.UpdateableClassifier)
    ...
    try {
      if (m_ClassifierTemplate instanceof weka_predictions.classifiers.misc.InputMappedClassifier) {
        m_Classifier = weka_predictions.classifiers.AbstractClassifier
   ...
   * @return the classifier template currently in use.
   */
  public weka_predictions.classifiers.Classifier getClassifierTemplate() {
    return m_ClassifierTemplate;
  }

  private void setTrainedClassifier(weka_predictions.classifiers.Classifier tc)
      throws Exception {

    // set the template
    weka_predictions.classifiers.Classifier newTemplate = null;
    String[] options = ((OptionHandler) tc).getOptions();
    newTemplate = weka_predictions.classifiers.AbstractClassifier.forName(tc.getClass()
        .getName(), options);
   ...
   */
  public weka_predictions.classifiers.Classifier getClassifier() {
    return m_Classifier;
  }
  ...
  public void setWrappedAlgorithm(Object algorithm) {
    ...
    if (!(algorithm instanceof weka_predictions.classifiers.Classifier)) {
      throw new IllegalArgumentException(algorithm.getClass() + " : incorrect "
          + "type of algorithm (Classifier)");
    }
    setClassifierTemplate((weka_predictions.classifiers.Classifier) algorithm);
  }
  ...
  private void handleIncrementalEvent() {
    ...
    if (m_incrementalEvent.getStatus() == InstanceEvent.FORMAT_AVAILABLE) {
      ...
      try {
        ...
        if (m_trainingSet == null || !m_trainingSet.equalHeaders(dataset)
          ...
          if (!(m_ClassifierTemplate instanceof weka_predictions.classifiers.UpdateableClassifier)
              && !(m_ClassifierTemplate instanceof weka_predictions.classifiers.misc.InputMappedClassifier)) {
          ...
          if (m_ClassifierTemplate instanceof weka_predictions.classifiers.misc.InputMappedClassifier) {
            m_trainingSet = ((weka_predictions.classifiers.misc.InputMappedClassifier) m_Classifier)
    ...
    try {
      ...
      if (m_ClassifierTemplate instanceof weka_predictions.classifiers.UpdateableClassifier
        ...
        ((weka_predictions.classifiers.UpdateableClassifier) m_Classifier)
    ...
    public void execute() {
      try {
        if (m_train != null) {
          ...
          weka_predictions.classifiers.Classifier classifierCopy = weka_predictions.classifiers.AbstractClassifier
              .makeCopy(m_ClassifierTemplate);
          ...
          if (classifierCopy instanceof weka_predictions.core.Drawable
              && m_graphListeners.size() > 0) {
            String grphString = ((weka_predictions.core.Drawable) classifierCopy).graph();
            int grphType = ((weka_predictions.core.Drawable) classifierCopy).graphType();
  ...
  public synchronized void acceptTestSet(TestSetEvent e) {
    ...
    weka_predictions.classifiers.Classifier classifierToUse = m_Classifier;
    ...
    if (classifierToUse != null && m_state == IDLE
      ...
      if (classifierToUse instanceof weka_predictions.classifiers.misc.InputMappedClassifier) {
        // make sure that we have the correct training header (if
        // InputMappedClassifier
        // is loading a model from a file).
        try {
          m_trainingSet = ((weka_predictions.classifiers.misc.InputMappedClassifier) classifierToUse)
      ...
      for (int i = 0; i < l.size(); i++) {
        if (l.size() > 1) {
          try {
            ...
            weka_predictions.classifiers.Classifier newC = weka_predictions.classifiers.AbstractClassifier
  ...
  protected void loadFromFile(File loadFrom) throws Exception {
    weka_predictions.classifiers.Classifier temp = null;
    ...
    if ((KOML.isPresent())
      ...
      temp = (weka_predictions.classifiers.Classifier) v.elementAt(0);
      if (v.size() == 2) {
      ...
      temp = (weka_predictions.classifiers.Classifier) is.readObject();
  ...
  public boolean eventGeneratable(String eventName) {
    ...
    if (eventName.compareTo("graph") == 0) {
      ...
      if (!(m_ClassifierTemplate instanceof weka_predictions.core.Drawable)) {
        return false;
      }
