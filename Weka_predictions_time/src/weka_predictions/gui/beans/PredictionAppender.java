...
import weka_predictions.clusterers.DensityBasedClusterer;
...
public class PredictionAppender extends JPanel implements DataSource,
  ...
  protected transient weka_predictions.gui.Logger m_logger;
  ...
  public void acceptClassifier(IncrementalClassifierEvent e) {
    weka_predictions.classifiers.Classifier classifier = e.getClassifier();
          ...
          if (m_dataSourceListeners.size() > 0 || m_testSetListeners.size() > 0) {
            ...
            for (int i = 0; i < testSet.numInstances(); i++) {
              ...
              if (tempInst.isMissing(tempInst.classIndex())
                  && !(classifier instanceof weka_predictions.classifiers.misc.InputMappedClassifier)) {
  ...
  public void acceptClusterer(BatchClustererEvent e) {
    if (m_dataSourceListeners.size() > 0 || m_trainingSetListeners.size() > 0
      ...
      weka_predictions.clusterers.Clusterer clusterer = e.getClusterer();
      String test;
  ...
  private Instances makeDataSetProbabilities(Instances insts, Instances format,
      weka_predictions.classifiers.Classifier classifier, String relationNameModifier)
      throws Exception {
    ...
    if (classifier instanceof weka_predictions.classifiers.misc.InputMappedClassifier) {
      format = ((weka_predictions.classifiers.misc.InputMappedClassifier) classifier)
    ...
    for (int i = 0; i < format.classAttribute().numValues(); i++) {
      weka_predictions.filters.unsupervised.attribute.Add addF = new weka_predictions.filters.unsupervised.attribute.Add();
      ...
      newInstances = weka_predictions.filters.Filter.useFilter(newInstances, addF);
    }
  ...
  private Instances makeDataSetClass(Instances insts, Instances structure,
    ...
    // adjust structure for InputMappedClassifier (if necessary)
    if (classifier instanceof weka_predictions.classifiers.misc.InputMappedClassifier) {
      structure = ((weka_predictions.classifiers.misc.InputMappedClassifier) classifier)
    ...
    Instances newInstances = weka_predictions.filters.Filter.useFilter(insts, addF);
    newInstances.setRelationName(insts.relationName() + relationNameModifier);
    return newInstances;
  }

  private Instances makeClusterDataSetProbabilities(Instances format,
      weka_predictions.clusterers.Clusterer clusterer, String relationNameModifier)
      throws Exception {
  ...
  private Instances makeClusterDataSetClass(Instances format,
    ...
    Instances newInstances = weka_predictions.filters.Filter.useFilter(format, addF);
    newInstances.setRelationName(format.relationName() + relationNameModifier);
    return newInstances;
  }
   ...
   */
  @Override
  public void setLog(weka_predictions.gui.Logger logger) {
    m_logger = logger;
  }
