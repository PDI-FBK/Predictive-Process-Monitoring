...
import weka_predictions.core.converters.ArffLoader;
import weka_predictions.core.converters.DatabaseLoader;
import weka_predictions.core.converters.FileSourcedConverter;
import weka_predictions.core.converters.Loader.StructureNotReadyException;
...
public class Loader extends AbstractDataSource implements Startable,
   ...
   * Loader
   */
  private weka_predictions.core.converters.Loader m_Loader = new ArffLoader();
    ...
    public void run() {
      ...
      try {
        ...
        if (instanceGeneration) {
          ...
          try {
            m_Loader.reset();
            m_Loader.setRetrieval(weka_predictions.core.converters.Loader.INCREMENTAL);
          ...
          m_Loader.reset();
          m_Loader.setRetrieval(weka_predictions.core.converters.Loader.BATCH);
   ...
   */
  public void setLoader(weka_predictions.core.converters.Loader loader) {
   ...
   */
  public weka_predictions.core.converters.Loader getLoader() {
    return m_Loader;
  }
  ...
  public void setWrappedAlgorithm(Object algorithm) {
    ...
    if (!(algorithm instanceof weka_predictions.core.converters.Loader)) {
      throw new IllegalArgumentException(algorithm.getClass() + " : incorrect "
          + "type of algorithm (Loader)");
    }
    setLoader((weka_predictions.core.converters.Loader) algorithm);
  }
  ...
  public boolean eventGeneratable(String eventName) {
    if (eventName.compareTo("instance") == 0) {
      if (!(m_Loader instanceof weka_predictions.core.converters.IncrementalConverter)) {
        return false;
      }
      if (m_dataSetEventTargets > 0) {
        return false;
    ...
    if (eventName.compareTo("dataSet") == 0) {
      if (!(m_Loader instanceof weka_predictions.core.converters.BatchConverter)) {
        return false;
      }
